...
static void
ilm_sync_net_down(const struct net *net)
{
...
bucket_restart:
		...
		for (j = 0; j < n->pos; j++) {
			...
			if (net_eq(rtnl_dereference(ilm->nhlfe)->net, net)) {
		...
		if (net_eq(&init_net, net)) {
			...
			for_each_net(tmp) {
				if (net_eq(tmp, &init_net))
					continue;
				ilm_sync_dev_down(tmp, dev, false);
			}
		} else
			ilm_sync_dev_down(&init_net, dev, false);
	}
